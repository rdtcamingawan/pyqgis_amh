###############################################################################
#  Amacan HMS Auto-Calibrator V29 (Portable Executable Version)
#
#  This version includes logic to make the script portable when packaged
#  with PyInstaller. It automatically detects the project folder relative
#  to the location of the .exe file.
#
#  - KEY CHANGES -
#  1. Added a `get_base_path()` function to find the application's directory.
#  2. Default paths for the project and observed data are now set automatically
#     at runtime, assuming the project folder is next to the executable.
#  3. This is the final version, incorporating all previous fixes and features.
###############################################################################
import os
import sys
import threading
...
def compute_nse(dss_filepath, dss_run_name, obs_df, target_subbasin, iteration_num):
    """
...
def run_calibration_workflow():
    ...
    while best_nse < target_nse and iteration < max_iterations:
        ...
        # Call the NSE function with the iteration number
    ...
    if best_nse >= target_nse:
        print(f"\nTarget NSE of {target_nse} reached in iteration {iteration}.")
...
root = Tk(); root.title("HMS Auto-Calibrate"); root.geometry("850x950")
...
# --- USER DEFAULT VALUES ARE SET HERE (NOW PORTABLE) ---

def get_base_path():
    """Function to determine the base path (works for both script and .exe)"""
...
# Construct default paths relative to the application's location
default_project_dir = os.path.join(base_path, "hms_edc_amacan_CALIBRATION")
default_obs_file = os.path.join(default_project_dir, "NSE_computation", "observed_data.xlsx")

hms_dir_var = StringVar(value="C:/Program Files/HEC/HEC-HMS/4.12")
project_dir_var = StringVar(value=default_project_dir)
...
obs_xl_var = StringVar(value=default_obs_file)
